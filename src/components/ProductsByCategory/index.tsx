      ...
      <div className="grid grid-cols-1 gap-y-4 sm:gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        {products.length > 0 ? 
          products.map((product) => (
            <a key={product.id} href={`/product/${product.attributes.slug}`} className="grid grid-cols-2 sm:block sm:h-full group p-4 rounded-lg bg-pink-50 shadow-lg sm:shadow-xl">
              ...
              <div className="flex flex-col ml-4 justify-center">
                <h3 className="sm:mt-4 text-lg font-semibold text-gray-700">{product.attributes.title}</h3>
                <p className="mt-1 text-lg font-medium text-brown-400">{formatPrice(product.attributes.price)}</p>
              </div>
            </a>
          ))
        :
          <div className="grid grid-cols-2 sm:block sm:h-full group p-4 rounded-lg bg-pink-50 shadow-lg sm:shadow-xl">
            <div className="w-40 h-40 sm:w-full aspect-w-1 sm:aspect-h-1 bg-gray-200 rounded-lg overflow-hidden xl:aspect-w-7 xl:aspect-h-6">
              
            </div>
            <div className="flex flex-col ml-4 justify-center">
              <h3 className="sm:mt-4 text-lg font-semibold text-gray-700">Sem produto disponível no momento</h3>
